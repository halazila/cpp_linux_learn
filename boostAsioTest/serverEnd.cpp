...
void ServerEnd::deadlineTimerHandle()
{
    for (auto it = clientMap.begin(); it != clientMap.end(); it++)
    {
        if (it->second.get() != nullptr)
        {
            ...
            {
                char str[128];
                sprintf(str, "line %d, Remote Endpoint: %s:%d lose connection after 5 heart beat\n", __LINE__,
                        it->second->sock.remote_endpoint().address().to_string().c_str(),
                        it->second->sock.remote_endpoint().port());
                std::cout << str << std::endl;
